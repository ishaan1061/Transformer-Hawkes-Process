...
class EventData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return self.time[idx], self.time_gap[idx], self.event_type[idx], self.sentence_embedding[idx]
...
def collate_fn(insts):
    ...
    time, time_gap, event_type, sentence_embedding = list(zip(*insts))
    time = pad_time(time)
    time_gap = pad_time(time_gap)
    event_type = pad_type(event_type)
    sentence_embedding = pad_embedding(sentence_embedding)
    return time, time_gap, event_type, sentence_embedding
